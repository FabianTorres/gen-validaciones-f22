...
class TestMatrixBuilder:
    def __init__(
        self,
        cache_parametros: dict,
        cache_ruts: list,
        cache_codigos: dict,
        cache_asts: dict,
        config_motor: dict = None,
    ):
        """
    ...
    def generar_matriz_pruebas(self, ast_tree, id_val):
        ...
        print(
            f"\n{'=' * 70}\n[{hora}] 🚀 [TEST_BUILDER] Inicio procesamiento: {id_val}\n{'=' * 70}"
        )
        ...
        def pre_scan(n):
            if not hasattr(n, "data"):
                ...
                if v.isdigit():
                    vars_principales.add(f"[{v}]")
                elif v.startswith("[") and v.endswith("]"):
                    vars_principales.add(v)
                return
            for h in getattr(n, "children", []):
                pre_scan(h)
        ...
        for hijo in getattr(ast_tree, "children", []):
            if getattr(hijo, "data", "") == "autocalculado":
                cod_obj = (
                    str(hijo.children[0]).replace("[", "").replace("]", "").strip()
                )
                visitados_inicial.add(cod_obj)
                break

        # ---> MEDICIÓN DEPENDENCIAS - DEGUB TEMPORAL <---
        t_dep = time.perf_counter()
        hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        print(f"[{hora}] 🔄 [TEST_BUILDER] Resolviendo dependencias recursivas...")

        # ---> ESCUDO FAIL-FAST (CAPTURA) <---
        try:
            # Le pasamos la lista inicial con el código objetivo ya "visitado"
            restricciones_leyes_universo, asts_dep = (
                self._resolver_dependencias_autocalculados(ast_tree, visitados_inicial)
            )
            ...
            return [
                {
                    ...
                    "error": str(e),
                    "estado_interno": "ERROR_AST_FALTANTE",
                }
            ]

        # debug temporal
        duracion_dep = time.perf_counter() - t_dep
        hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        print(
            f"[{hora}] ⏱️ [TEST_BUILDER] Dependencias listas en {duracion_dep:.2f}s (ASTs detectados: {len(asts_dep)})"
        )

        for restriccion in restricciones_leyes_universo:
            self.motor.agregar_restriccion_base(restriccion)
        ...
        if not estrategia:
            return [
                {
                    "id_validacion": id_val,
                    "error": f"Aún no hay estrategia para: {tipo_regla}",
                }
            ]
        ...
        hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        print(
            f"[{hora}] 🎯 [TEST_BUILDER] Ejecutando estrategia '{estrategia.__class__.__name__}'..."
        )

        casos = estrategia.generar_casos(ast_tree, id_val)

        duracion_total = time.perf_counter() - t_global
        hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        print(
            f"\n[{hora}] 🏁 [TEST_BUILDER] Fin {id_val}. Tiempo TOTAL: {duracion_total:.2f}s ({len(casos)} casos)\n{'=' * 70}\n"
        )
        return casos
                ...
                if tipo in ("CODIGO", "VARIABLE_CORCHETE"):
                    ...
                    if cod_limpio.isdigit() and cod_limpio not in visitados:
                        ...
                        if info_cod.get("autocalculado", False):
                            ...
                            if not ast_dict:
                                raise ValueError(
                                    f"Falta el arbol AST para el código {clave_ast}. Verifica que su validación haya sido procesada previamente."
                                )
