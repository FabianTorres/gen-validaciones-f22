...
class CalculationBuilder(BaseStrategy):
    ...
    def generar_casos(self, ast_tree, id_val):
        ...
        if not nodo_principal:
            return [
                {"id_validacion": id_val, "error": "No se encontró nodo de cálculo."}
            ]
        ...
        if hasattr(ast_tree, "data") and ast_tree.data == "validacion":
            for hijo in ast_tree.children:
                if hasattr(hijo, "data") and hijo.data in (
                    "cota",
        ...
        for nombre, var_z3 in self.motor.variables_memoria.items():
            ...
            if (
                not self.config_motor.get("usar_decimales", False)
                and nombre.startswith("[")
                and nombre.endswith("]")
            ):
        ...
        nodos_func = self._encontrar_nodos_tipo(
            ast_tree, "funcion_matematica"
        ) + self._encontrar_nodos_tipo(ast_tree, "funcion_directa")
        ...
        for nodo_func in nodos_func:
            ...
            desc_sufijo = (
                f" (Instancia {func_current[func_name]})"
                if func_totals[func_name] > 1
                else ""
            )

            if nodo_func.data == "funcion_matematica":
                args_limpios = [
                    h for h in nodo_func.children[2].children if str(h) != ";"
                ]
            else:
            ...
            if not camino_base:
                ...
                if nodos_cond:
                    condicion_principal_z3 = self.evaluador.evaluar(
                        nodos_cond[0].children[0]
                    )
                else:
                    nodos_trail = self._encontrar_nodos_tipo(ast_tree, "caso_trailing")
                    if nodos_trail:
                        condicion_principal_z3 = self.evaluador.evaluar(
                            nodos_trail[0].children[-1]
                        )
            ...
            def aislar_y_ejecutar(restriccion_frontera, formateador_lambda):
                ...
                if condicion_principal_z3 is not None:
                    ...
                    print(
                        f"[{hora}] ➡️ [CALC_BUILDER] Intento 1: Forzando Happy Path + Frontera..."
                    )
                ...
                hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(
                    f"[{hora}] ⚠️ [CALC_BUILDER] Intento 1 fue UNSAT. Ejecutando Intento 2: Fallback (frontera aislada)..."
                )
                return self._ejecutar_escenario_aislado(
                    base_cond + [restriccion_frontera], formateador_lambda
                )
            ...
            if func_name == "MIN":
                ...
                casos.append(
                    aislar_y_ejecutar(
                        z3_arg1 <= (z3_arg2 - gap),
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_MIN{s}_IZQ",
                            f"El límite MIN{d} toma el valor izquierdo garantizando su ruta.",
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_MIN{s}_DER",
                            f"El límite MIN{d} toma el valor derecho garantizando su ruta.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_MAX{s}_IZQ",
                            f"El límite MAX{d} toma el valor izquierdo garantizando su ruta.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_MAX{s}_DER",
                            f"El límite MAX{d} toma el valor derecho garantizando su ruta.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_POS{s}_MAYOR_CERO",
                            f"El valor interno de POS{d} es positivo en su ruta correcta.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_POS{s}_MENOR_CERO",
                            f"El valor interno de POS{d} es negativo, forzando a 0.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_NEG{s}_MENOR_CERO",
                            f"El valor interno de NEG{d} es negativo, retornando valor absoluto.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_NEG{s}_MAYOR_CERO",
                            f"El valor interno de NEG{d} es positivo, forzando a 0.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"ABS{s}_ENTRADA_NEGATIVA",
                            f"El valor interno de ABS{d} es negativo, forzando conversión a positivo.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
                ...
                casos.append(
                    aislar_y_ejecutar(
                        ...
                        lambda s=sufijo, d=desc_sufijo: self._resolver_y_formatear(
                            id_val,
                            f"ABS{s}_ENTRADA_POSITIVA",
                            f"El valor interno de ABS{d} es positivo, manteniendo su valor.",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        ),
        ...
        for idx, (cond_node, cond_ast, expr_ast) in enumerate(condiciones_a_evaluar, 1):
            ...
            for i, var_verdadera in enumerate(variaciones_verdaderas, 1):
                ...
                tag_limite = (
                    f"_{var_verdadera['tag_limite']}"
                    if "tag_limite" in var_verdadera
                    else ""
                )
                ...
                casos.append(
                    self._ejecutar_escenario_aislado(
                        ...
                        lambda v=var_verdadera, s=sufijo, n=nivel, t=tag_limite: (
                            self._resolver_y_formatear(
                                id_val,
                                f"CALCULO_VERDADERO_{n}{s}{t}",
                                v["desc"],
                                "VERIFICAR_AUTOCALCULO",
                                codigo_objetivo,
                                ast_tree=ast_tree,
            ...
            for i, var_falsa in enumerate(variaciones_falsas, 1):
                # CORRECCIÓN: Usamos 'tag_limite' en lugar de 'bva_tag'
                tag_limite = (
                    f"_{var_falsa['tag_limite']}" if "tag_limite" in var_falsa else ""
                )
                sufijo = f"_{i}" if len(variaciones_falsas) > 1 else ""

                # ---> ANTI-MASKING: Intentamos inflar la rama muerta <---
                def generar_falso_anti_masking(
                    v=var_falsa, s=sufijo, n=nivel, t=tag_limite, expr=expr_ast
                ):
                    try:
                        hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                        print(
                            f"[{hora}] ➡️ [CALC_BUILDER] Probando Anti-Masking para rama SINO..."
                        )
                        ...
                        res = self._resolver_y_formatear(
                            id_val,
                            f"CALCULO_FALSO_{n}_SINO{s}{t}",
                            v["desc"] + " [Anti-Masking Inyectado]",
                            "VERIFICAR_AUTOCALCULO",
                            codigo_objetivo,
                            ast_tree=ast_tree,
                        )
                    ...
                    hora = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    print(
                        f"[{hora}] ⚠️ [CALC_BUILDER] Anti-Masking rama SINO fue UNSAT. Aplicando Falso normal..."
                    )
        ...
        for c in casos:
            if c is not None:
                ...
                # 2. Otros errores estructurales (ej. falta de AST) sí se bloquean o descartan
                    ...
                    print(
                        f"⚠️ Aviso en {id_val}: Escenario descartado internamente. Motivo: {c['error']}"
                    )
        ...
        return (
            ...
            if casos_validos
            else [
                {
                    ...
                    "error": "Inconsistencia matemática en todas las ramas.",
                }
            ]
        )
    ...
    def _obtener_camino_a_nodo(self, nodo_objetivo, ast_tree):
        ...
        for cond_node in nodos_condicional:
            ...
            en_entonces = len(cond_node.children) >= 2 and self._contiene_nodo(
                cond_node.children[1], nodo_objetivo
            )
            en_sino = len(cond_node.children) >= 3 and self._contiene_nodo(
                cond_node.children[2], nodo_objetivo
            )

            if not en_entonces and not en_sino and var_asociada:
                en_entonces = len(cond_node.children) >= 2 and self._contiene_texto(
                    cond_node.children[1], var_asociada
                )
                en_sino = len(cond_node.children) >= 3 and self._contiene_texto(
                    cond_node.children[2], var_asociada
                )
    ...
    def _desglosar_condicion_verdadera(self, z3_cond):
        ...
        if kind == z3.Z3_OP_OR:
            ...
            variaciones_hijos = [
                self._desglosar_condicion_verdadera(h) for h in z3_cond.children()
            ]
            ...
            if ACTIVAR_LIMITES_CALCULOS and kind in (
                z3.Z3_OP_LT,
                z3.Z3_OP_LE,
                z3.Z3_OP_GT,
                z3.Z3_OP_GE,
                ...
                if kind == z3.Z3_OP_LE:
                    variaciones.append(
                        {
                            "restriccion": izq == der,
                            "desc": "Evaluación en el límite exacto (<=).",
                            "tag_limite": "EN_EL_LIMITE",
                        }
                    )
                    variaciones.append(
                        {
                            "restriccion": izq == der - 1,
                            "desc": "Evaluación bajo el límite (<=).",
                            "tag_limite": "BAJO_EL_LIMITE",
                        }
                    )
                    ...
                    variaciones.append(
                        {
                            "restriccion": izq == der + 1,
                            "desc": "Evaluación sobre el límite (>=).",
                            "tag_limite": "SOBRE_EL_LIMITE",
                        }
                    )
    ...
    def _desglosar_condicion_falsa(self, z3_cond):
        ...
        if kind == z3.Z3_OP_AND:
            ...
            variaciones_hijos = [
                self._desglosar_condicion_falsa(h) for h in z3_cond.children()
            ]
            combinaciones = list(itertools.product(*variaciones_hijos))
            ...
            if ACTIVAR_LIMITES_CALCULOS and kind in (
                ...
                if kind == z3.Z3_OP_LE:  # Falso de <= es >
                    variaciones.append(
                        {
                            "restriccion": izq == der + 1,
                            "desc": "Condición falla sobre el límite (<=).",
                            "tag_limite": "SOBRE_EL_LIMITE",
                        }
                    )
                    ...
                    variaciones.append(
                        {
                            "restriccion": izq == der - 1,
                            "desc": "Condición falla bajo el límite (>=).",
                            "tag_limite": "BAJO_EL_LIMITE",
                        }
                    )
                    ...
                    variaciones.append(
                        {
                            "restriccion": izq == der,
                            "desc": "Condición falla en el límite exacto (<).",
                            "tag_limite": "EN_EL_LIMITE",
                        }
                    )
                    variaciones.append(
