...
from src.api.schemas import (
    ...
    CasoQA,
    GuardarReglaRequest,
    GuardarReglaResponse,
    BatchDependenciasRequest,
    CargarAstsRequest,
    HistorialItem,
...
app = FastAPI(
    title="Motor Generador Validaciones F22", version="2.1.0", lifespan=lifespan
)
...
async def endpoint_normalizar(req: FormulaRequest):
    ...
    return NormalizacionResponse(
        estado="EXITO", texto_formateado=resultado["texto_formateado"]
    )
...
async def endpoint_generar_casos(req: FormulaRequest):
    ...
    # 1. Fase 1: Parseo
    res_norm = await run_in_threadpool(
        normalizar_y_validar,
        req.formula_cruda,
        req.id_validacion,
        codigos_at,
    ...
    if not casos_generados:
        raise HTTPException(
            status_code=400, detail="Error de contradicción lógica en SMT Solver."
        )
    ...
    if "error" in casos_generados[0] and "inputs" not in casos_generados[0]:
        raise HTTPException(
            status_code=400,
            detail=casos_generados[0].get("error", "Error fatal en el generador."),
        )

    # 3. Fase 3: Optimización (Sobre los casos ya estructurados por Fase 2)
    reductor = ReductorCasos()
    casos_optimizados, estadisticas = await run_in_threadpool(
        reductor.procesar_casos, casos_generados
    )

    # ==========================================

    stats_regla = estadisticas.get(
        req.id_validacion.split(".")[0],
        {"originales": len(casos_generados), "optimizados": len(casos_optimizados)},
    )
    reduccion = (
        (
            (stats_regla["originales"] - stats_regla["optimizados"])
            / stats_regla["originales"]
            * 100
        )
        if stats_regla["originales"] > 0
        else 0.0
    )
    ...
    for caso in casos_generados:
        if caso.get("error") and "Falta el AST" in caso.get("error", ""):
            raise HTTPException(
                status_code=400, detail=f"Generación bloqueada: {caso.get('error')}"
            )
...
async def obtener_catalogo(
    tipo: str, at: int = Query(2026, description="Año Tributario (ej. 2026)")
):
    """
    ...
    if tipo_limpio in ["parametro", "parametros"]:
        return cache.parametros.get(at, {})
    elif tipo_limpio in ["codigo", "codigos"]:
        return cache.codigos.get(at, {})
    elif tipo_limpio in ["rut", "ruts"]:
        return cache.ruts.get(at, [])
    elif tipo_limpio in ["mensaje", "mensajes"]:
        return cache.mensajes.get(at, {})
    else:
        raise HTTPException(status_code=404, detail="Tipo de catálogo no reconocido.")
...
async def agregar_parametro(req: ParametroItem):
    ...
    return {
        "estado": "EXITO",
        "mensaje": f"Parámetro {req.id} guardado/actualizado en AT {req.at}",
    }
...
async def agregar_codigo(req: CodigoItem):
    ...
    return {
        "estado": "EXITO",
        "mensaje": f"Código {req.id} guardado/actualizado en AT {req.at}",
    }
...
async def agregar_mensaje(req: MensajeItem):
    ...
    return {
        "estado": "EXITO",
        "mensaje": f"Mensaje {req.id} guardado/actualizado en AT {req.at}",
    }
...
async def agregar_rut(req: RutItem):
    ...
    return {
        "estado": "EXITO",
        "mensaje": f"RUT {req.id} guardado/actualizado en AT {req.at}",
    }
...
async def batch_parametros(req: BatchParametros):
    items_dict = [
        {**item.model_dump(exclude_none=True), "tipo": "parametro"}
        for item in req.items
    ]
    await cache.carga_masiva_catalogos(items_dict)
    return {
        "estado": "EXITO",
        "mensaje": f"Se procesaron {len(items_dict)} parámetros.",
    }
...
async def batch_codigos(req: BatchCodigos):
    ...
    items_dict = [
        {**item.model_dump(exclude_none=True), "tipo": "codigo"} for item in req.items
    ]
    await cache.carga_masiva_catalogos(items_dict)
...
async def batch_mensajes(req: BatchMensajes):
    items_dict = [
        {**item.model_dump(exclude_none=True), "tipo": "mensaje"} for item in req.items
    ]
    await cache.carga_masiva_catalogos(items_dict)
...
async def batch_ruts(req: BatchRuts):
    items_dict = [
        {**item.model_dump(by_alias=True, exclude_none=True), "tipo": "rut"}
        for item in req.items
    ]
    await cache.carga_masiva_catalogos(items_dict)
...
async def exportar_csv_selenium(casos: List[CasoQA], at: int = Query(2026)):
    ...
    codigos_at = cache.codigos.get(at, {})
    casos_dict = [caso.model_dump(exclude_none=True) for caso in casos]
    exportador = ExportadorCSV(mensajes_at, codigos_at)
...
async def guardar_regla_aprobada(req: GuardarReglaRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al guardar en Cosmos DB: {str(e)}"
        )
...
async def obtener_historial_reglas(at: int = Query(2026, description="Año Tributario")):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al consultar historial: {str(e)}"
        )
...
async def obtener_detalle_regla(
    ...
    """
    try:
        detalle_unificado = await repo_historial.obtener_detalle_completo(
            id_validacion, version, at
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al obtener el detalle de la regla: {str(e)}"
        )
...
async def exportar_lote_csv(
    ...
    try:
        ...
        if not casos_totales:
            raise HTTPException(
                status_code=404,
                detail=f"No se encontraron casos para el prefijo '{prefijo}' en el AT {at}.",
            )
        ...
        if nombre_seguro.endswith("_"):
            nombre_seguro = nombre_seguro[:-1]

        return Response(
            content=csv_str,
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=lote_{nombre_seguro}_AT{at}.csv"
            },
        )
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al exportar lote masivo: {str(e)}"
        )
...
async def exportar_matriz_individual(
    ...
    """
    try:
        casos = await repo_historial.obtener_matriz_especifica(
            id_validacion, version, at
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al exportar matriz individual: {str(e)}"
        )
...
async def buscar_dependencias(
    codigo: str, at: int = Query(2026, description="Año Tributario")
):
    """
    ...
    reglas_asociadas = await repo_historial.buscar_dependencias_por_codigo(
        codigo_limpio, at
    )

    if not reglas_asociadas:
        return {
            "mensaje": f"No se encontraron reglas que afecten al código {codigo_limpio} en el AT {at}",
            "resultados": [],
        }

    return {
        "codigo_objetivo": codigo_limpio,
        "at": at,
        "total_encontradas": len(reglas_asociadas),
        "resultados": reglas_asociadas,
    }
...
async def buscar_dependencias_lote(req: BatchDependenciasRequest):
    ...
    if not req.codigos:
        raise HTTPException(
            status_code=400, detail="Debe proporcionar al menos un código."
        )

    codigos_limpios = [
        f"[{c.replace('[', '').replace(']', '').strip()}]" for c in req.codigos
    ]
...
async def cargar_memoria_asts(req: CargarAstsRequest):
    ...
    try:
        ...
        return {
            "estado": "EXITO",
            "mensaje": f"Se cargaron {total_cargados} ASTs (AT {req.at}) a la memoria RAM.",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error al hidratar memoria con ASTs: {str(e)}"
        )
